...
class User
  ...
  def self.create_user(json)
    ...
    ret = { 
      :username => json['user'], :password_hash => password, :password_salt => salt, :name => json['name'], :email => json['email']
    }
    ret 
  end
...
post '/register' do
  ret   = Array.new
  jdata = {:name => params[:name],:user => params[:username],:pwd => params[:password],:email => params[:email]}.to_json
